...
namespace Oxide.Plugins
{
    ...
    public class DeathmatchSoccer : RustPlugin
    {
        ...
        private void CmdMyEmblems(BasePlayer player, string command, string[] args)
        {
            if (EmblemEditor == null)
            {
                SendReply(player, "ERROR: EmblemEditor plugin not loaded!");
            ...
            try
            {
                // Get player's emblems from EmblemEditor
                var emblemsData = EmblemEditor.Call("GetPlayerEmblems", player.userID);
                
                if (emblemsData == null)
                {
                    SendReply(player, "You don't have any saved emblems yet!");
                    SendReply(player, "Use /emblem to create your first emblem.");
                    return;
                }
                
                // Get equipped emblem ID
                string equippedUrl = (string)EmblemEditor.Call("GetEquippedEmblem", player.userID);
                
                // Show emblem selection UI
                ShowEmblemsUI(player, emblemsData, equippedUrl);
            }
            catch (Exception ex)
            {
                SendReply(player, "ERROR: Failed to load emblems");
                Puts($"[MyEmblems] Error: {ex.Message}");
            }
        }
        
        private void ShowEmblemsUI(BasePlayer player, object emblemsData, string equippedUrl)
        {
            ...
            elements.Add(new CuiPanel
            {
                ...
                CursorEnabled = true
            }, "Overlay", "EmblemsPanel");
            
            // Title
            elements.Add(new CuiLabel
            {
                Text = { Text = "My Emblems", FontSize = 24, Align = TextAnchor.MiddleLeft, Color = "1 1 1 1" },
                RectTransform = { AnchorMin = "0.05 0.9", AnchorMax = "0.7 0.98" }
            }, "EmblemsPanel");
            
            // Close button
            elements.Add(new CuiButton
            {
                Button = { Command = "closeemblems", Color = "0.8 0.2 0.2 1" },
                RectTransform = { AnchorMin = "0.92 0.92", AnchorMax = "0.98 0.98" },
                Text = { Text = "X", FontSize = 16, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" }
            }, "EmblemsPanel");
            
            // Parse emblems data (expecting List<Dictionary> or similar structure)
            var emblems = emblemsData as List<object>;
            if (emblems == null || emblems.Count == 0)
            {
                elements.Add(new CuiLabel
                {
                    Text = { Text = "No emblems found", FontSize = 14, Align = TextAnchor.MiddleCenter, Color = "0.8 0.8 0.8 1" },
            ...
            // Grid layout: 4 emblems per row
            int columns = 4;
            float emblemWidth = 0.18f;
            float emblemHeight = 0.25f;
            float spacing = 0.03f;
            float startX = 0.05f;
            float startY = 0.65f;
            
            int index = 0;
            foreach (var emblemObj in emblems)
            {
                var emblem = emblemObj as Dictionary<string, object>;
                if (emblem == null) continue;
                
                string emblemId = emblem.ContainsKey("id") ? emblem["id"].ToString() : "";
                string emblemUrl = emblem.ContainsKey("url") ? emblem["url"].ToString() : "";
                
                if (string.IsNullOrEmpty(emblemId) || string.IsNullOrEmpty(emblemUrl)) continue;
                
                // Calculate position
                int row = index / columns;
                int col = index % columns;
                
                float minX = startX + (col * (emblemWidth + spacing));
                float maxX = minX + emblemWidth;
                float maxY = startY - (row * (emblemHeight + spacing));
                float minY = maxY - emblemHeight;
                
                string panelName = $"EmblemItem_{index}";
                bool isEquipped = (emblemUrl == equippedUrl);
                
                // Emblem container panel
                elements.Add(new CuiPanel
                {
                    Image = { Color = isEquipped ? "0.2 0.8 0.2 0.3" : "0.2 0.2 0.2 0.5" },
                    RectTransform = { AnchorMin = $"{minX} {minY}", AnchorMax = $"{maxX} {maxY}" }
                }, "EmblemsPanel", panelName);
                
                // Register emblem with ImageLibrary if not already
                string imageLibId = $"Emblem_Preview_{emblemId}";
                if (ImageLibrary != null)
                {
                    ImageLibrary.Call("AddImage", emblemUrl, imageLibId);
                }
                
                // Emblem image (top 75% of container)
                string imgPng = GetImg(imageLibId);
                if (!string.IsNullOrEmpty(imgPng))
                {
                    elements.Add(new CuiElement
                    {
                        ...
                        Components =
                        {
                            new CuiRawImageComponent { Png = imgPng, Color = "1 1 1 1" },
                            new CuiRectTransformComponent { AnchorMin = "0.05 0.3", AnchorMax = "0.95 0.95" }
                        }
                    });
                }
                else
                {
                    ...
                    elements.Add(new CuiLabel
                    {
                        Text = { Text = "Loading...", FontSize = 10, Align = TextAnchor.MiddleCenter, Color = "0.7 0.7 0.7 1" },
                        RectTransform = { AnchorMin = "0.05 0.3", AnchorMax = "0.95 0.95" }
                    }, panelName);
                }
                
                // Equip button (bottom 25% of container)
                string buttonText = isEquipped ? "✓ Equipped" : "Equip";
                string buttonColor = isEquipped ? "0.2 0.6 0.2 1" : "0.4 0.6 0.8 1";
                
                elements.Add(new CuiButton
                {
                    Button = { Command = $"equipemblem {emblemId}", Color = buttonColor },
                    RectTransform = { AnchorMin = "0.1 0.05", AnchorMax = "0.9 0.25" },
                    Text = { Text = buttonText, FontSize = 11, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" }
                }, panelName);
            ...
            elements.Add(new CuiLabel
            {
                Text = { Text = "Click 'Equip' to set an emblem as your active emblem", FontSize = 12, Align = TextAnchor.MiddleCenter, Color = "0.7 0.7 0.7 1" },
        ...
        [ConsoleCommand("closeemblems")]
        private void CmdCloseEmblems(ConsoleSystem.Arg arg)
        {
            var player = arg.Player();
            if (player == null) return;
            
            CuiHelper.DestroyUi(player, "EmblemsPanel");
        }
        
        [ConsoleCommand("equipemblem")]
        private void CmdEquipEmblem(ConsoleSystem.Arg arg)
        {
            var player = arg.Player();
            if (player == null) return;
            
            string emblemId = arg.GetString(0);
            if (string.IsNullOrEmpty(emblemId))
            {
                SendReply(player, "ERROR: Invalid emblem ID");
                return;
            }
            ...
            try
            {
                // Call EmblemEditor to equip the emblem
                var result = EmblemEditor.Call("EquipEmblemById", player.userID, emblemId);
                
                if (result == null || !(bool)result)
                {
                    SendReply(player, "Failed to equip emblem. It may have been deleted.");
                    return;
                }
                
                SendReply(player, "✓ Emblem equipped successfully!");
                
                // If player owns a team, update team emblem
                if (playerTeamAssignments.ContainsKey(player.userID))
                {
                    string teamID = playerTeamAssignments[player.userID];
                    if (customTeams.ContainsKey(teamID))
                    {
                        ...
                        if (!string.IsNullOrEmpty(emblemUrl))
                        {
                            ...
                            if (ImageLibrary != null)
                            {
                                string teamEmblemId = $"Team_Emblem_{teamID}";
                                ImageLibrary.Call("AddImage", emblemUrl, teamEmblemId);
                            }
                            
                            SendReply(player, "Your team emblem has been updated too!");
                        }
                    }
                }
                
                // Close and reopen UI to show updated equipped status
                CuiHelper.DestroyUi(player, "EmblemsPanel");
            ...
            {
                SendReply(player, "ERROR: Failed to equip emblem");
                Puts($"[EquipEmblem] Error: {ex.Message}");
            }
        }
        ...
        private void GiveKit(BasePlayer player, string role)
        {
            ...
            if (hasCustomTeam)
            {
                ...
                // Give bonus weapon FIRST (in empty belt) - always given regardless of mode
                GiveBonusWeapon(player);
                
                // Then give Soccer Mode weapons if in Soccer Mode (fills remaining slots)
                if (gameMode == "soccer")
                {
